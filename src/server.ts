...
import { createSabrAudioStream, type StreamResults } from './sabr-stream-factory.js';
...
import type { SearchType } from './metadata-types.js';
...
interface TrackBuffer {
  ...
  /** Aborts the underlying download (SABR stream or direct fetch) on eviction. */
  abort?: () => void;
  /** Monotonic timestamp (ms) when the download was started. */
  startedAt: number;
}
 ...
 */
function evictOldestStream(): void {
  let oldest: TrackBuffer | undefined;
  let oldestKey: string | undefined;

  for (const [key, track] of tracks) {
    if (track.done) continue; // skip completed downloads
  ...
  if (!oldest || !oldestKey) return;

  console.log(
    `[stream] evicting oldest active stream ${oldestKey} ` +
...
async function startSabr(videoId: string, options: SabrPlaybackOptions): Promise<EngineResult> {
...
async function startEngine(
  ...
  if (engine === 'sabr') return startSabr(videoId, options);
  try {
    return await createDirectAudioStream(videoId);
  } catch (e) {
    console.warn(
      `[stream] [${key}] ios engine failed, falling back to sabr:`,
 ...
 */
function getOrStartTrack(videoId: string, options: SabrPlaybackOptions, engine: StreamEngine): TrackBuffer {
  ...
  (async () => {
    try {
      ...
      track.done = true;
      if (track.totalSize === undefined) track.totalSize = track.downloadedLength;
...
async function waitForTrackReady(track: TrackBuffer): Promise<void> {
  if (track.ready) {
...
async function waitForTotalSize(track: TrackBuffer): Promise<number> {
  if (track.totalSize !== undefined) return track.totalSize;
  if (track.done) return track.downloadedLength;
 ...
 */
function readBufferedSlice(track: TrackBuffer, start: number, end: number): Buffer {
  const parts: Buffer[] = [];
  let pos = 0;
...
app.get('/stream/:id', async (req, res) => {
  ...
  const options: SabrPlaybackOptions = { ...DEFAULT_OPTIONS };
  ...
  if (typeof req.query.audioQuality === 'string') {
    options.audioQuality = req.query.audioQuality as any;
  }
  ...
  if (!range) {
    if (track.totalSize !== undefined) res.setHeader('Content-Length', track.totalSize);
