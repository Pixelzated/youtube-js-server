...
const metadataCache = new TtlCache(THIRTY_MINUTES_MS, 1000);
const albumCache = new TtlCache(THIRTY_MINUTES_MS, 500);
const artistCache = new TtlCache(THIRTY_MINUTES_MS, 500);
// Search results/rankings shift more often, so a shorter TTL.
const searchCache = new TtlCache(FIVE_MINUTES_MS, 300);
...
const playlistCache = new TtlCache(THIRTY_MINUTES_MS, 200);
// ---------------------------------------------------------------------------
// Small extraction helpers
// ---------------------------------------------------------------------------
function mapThumbnails(thumbs) {
...
function idFromEndpoint(endpoint) {
    ...
    return (p.videoId ?? p.browseId ?? p.playlistId);
}
function makeEntity(name, id) {
    return { name: name ?? 'Unknown', ...(id ? { id } : {}) };
}
function makeDuration(seconds, text) {
...
async function searchUncached(query, type) {
 ...
 */
function mapMusicResponsiveListItem(item, isMusic) {
    ...
    const artist = makeEntity(firstArtist?.name, firstArtist?.channel_id ?? idFromEndpoint(firstArtist?.endpoint));
    // Channel: same source as artist for music; for video rows the author is
    // the uploading channel.
    const channel = makeEntity(firstArtist?.name ?? item.authors?.[0]?.name, firstArtist?.channel_id ?? item.authors?.[0]?.channel_id ?? idFromEndpoint(firstArtist?.endpoint));
...
async function getMetadataUncached(videoId) {
    const innertube = await getMetadataInnertube();
    let info;
    try {
    ...
    const match = upNext?.contents
        ?.filter((c) => c.is(YTNodes.PlaylistPanelVideo))
 ...
 */
async function getMetadataFromWeb(videoId) {
    const innertube = await getMetadataInnertube();
    let info;
    try {
...
async function getAlbumUncached(albumId) {
    const innertube = await getMetadataInnertube();
    let album;
    try {
    ...
    if (header?.is(YTNodes.MusicDetailHeader)) {
        ...
        if (h.author)
            artist = makeEntity(h.author.name, h.author.channel_id);
    }
    ...
    const tracks = (album.contents ?? []).map((item, idx) => {
        const id = item.id ?? idFromEndpoint(item.endpoint);
        return {
            id: id ?? '',
...
async function getArtistUncached(artistId) {
    const innertube = await getMetadataInnertube();
    let artist;
    try {
 ...
 */
export async function getPlaylist(playlistId, options = {}) {
    if (options.maxItems !== undefined)
        return getPlaylistUncached(playlistId, options.maxItems);
...
async function getPlaylistUncached(playlistId, maxItems) {
    ...
    for (;;) {
        ...
        for (const item of items) {
            ...
            const entry = mapPlaylistItem(item, allEntries.length + 1);
            if (!entry)
                continue;
        ...
        if (maxItems !== undefined && allEntries.length >= maxItems)
            break;
        if (!page.has_continuation)
            break;
        if (pageIndex >= MAX_PLAYLIST_PAGES) {
            console.warn(`[playlist] reached MAX_PLAYLIST_PAGES (${MAX_PLAYLIST_PAGES}) for ${id}; ` +
