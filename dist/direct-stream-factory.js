 ...
 */
let directInnertube;
async function getDirectInnertube() {
    if (directInnertube)
...
async function resolveDirectFormat(videoId, isRetryAfterSessionReset = false) {
    ...
    try {
        ...
        if (isRetryAfterSessionReset)
            throw e;
        console.warn(`[direct-stream] [${videoId}] getBasicInfo failed, session looks stale — resetting and retrying once:`, e instanceof Error ? e.message : e);
        resetDirectInnertube();
        return resolveDirectFormat(videoId, true);
    }
    const audioFormats = (info.streaming_data?.adaptive_formats || []).filter((f) => f.has_audio && !f.has_video && f.url);
    if (!audioFormats.length) {
        if (!isRetryAfterSessionReset) {
            console.warn(`[direct-stream] [${videoId}] no directly-fetchable audio formats returned, session looks stale — ` +
    ...
    return {
        ...
        sabrFormat: buildSabrFormat(best)
    };
}
 ...
 */
const FIRST_CHUNK_RETRY_BUDGET = {
    maxAttempts: 4,
    baseDelayMs: 200,
    maxDelayMs: 500,
    refreshAtAttempt: 3
 ...
 */
const LATER_CHUNK_RETRY_BUDGET = {
    maxAttempts: 5,
    baseDelayMs: 250,
    maxDelayMs: 1200,
    refreshAtAttempt: 3
 ...
 */
async function fetchRangeWithRetry(urlRef, videoId, rangeHeader, baseHeaders, signal, budget) {
    let lastError;
    for (let attempt = 1; attempt <= budget.maxAttempts; attempt++) {
        try {
            ...
            if (attempt < budget.maxAttempts) {
                ...
                console.warn(`[direct-stream] range fetch attempt ${attempt}/${budget.maxAttempts} failed ` +
                    `(${rangeHeader}), retrying in ${delay}ms:`, e instanceof Error ? e.message : e);
    ...
    console.error(`[direct-stream] range fetch permanently failed (${rangeHeader}) after ${budget.maxAttempts} attempts:`, lastError);
 ...
 */
function createChunkedRangeStream(urlRef, videoId, contentLength, headers, signal, firstChunk, startPosition) {
    ...
    return new ReadableStream({
        ...
        async pull(controller) {
            ...
            const buf = await fetchRangeWithRetry(urlRef, videoId, `bytes=${position}-${end}`, headers, signal, LATER_CHUNK_RETRY_BUDGET);
            position = end + 1;
            controller.enqueue(new Uint8Array(buf));
        }
    });
}
export async function createDirectAudioStream(videoId) {
    ...
    const firstChunkBuf = await fetchRangeWithRetry(urlRef, videoId, `bytes=0-${firstChunkEnd}`, headers, abortController.signal, FIRST_CHUNK_RETRY_BUDGET);
    const audioStream = createChunkedRangeStream(urlRef, videoId, resolved.contentLength, headers, abortController.signal, new Uint8Array(firstChunkBuf), firstChunkEnd + 1);
    return {
        streamResults: {
            audioStream,
...
//# sourceMappingURL=direct-stream-factory.js.map