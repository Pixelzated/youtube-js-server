 ...
 */
let streamingInnertube;
async function getStreamingInnertube() {
    if (streamingInnertube)
 ...
 */
function resetStreamingInnertube() {
 ...
 */
function buildContentPlaybackContext(innertube) {
    return {
        vis: 0,
        splay: false,
        lactMilliseconds: '-1',
 ...
 */
const STREAMING_PLAYER_CLIENTS = ['YTMUSIC', 'WEB'];
/**
 ...
 */
async function fetchStreamingPlayerResponse(innertube, videoId, preferredClient, reloadPlaybackContext) {
    ...
    for (const client of clients) {
        ...
        try {
            ...
            if (playerResponse.streaming_data?.server_abr_streaming_url)
                return lastResult;
            console.warn(`[SABR] [${videoId}] ${client} player response has no server_abr_streaming_url`);
        }
        catch (e) {
            lastError = e;
            console.warn(`[SABR] [${videoId}] ${client} player request failed:`, e instanceof Error ? e.message : e);
        }
    }
    if (!lastResult) {
 ...
 */
export async function createSabrAudioStream(videoId, options, isRetryAfterSessionReset = false) {
    ...
    const { playerResponse, client: playerClient } = await fetchStreamingPlayerResponse(innertube, videoId);
    ...
    if (!playerResponse.streaming_data?.server_abr_streaming_url) {
        ...
        console.warn(`[SABR] [${videoId}] server_abr_streaming_url missing from player response — ` +
    ...
    const videoPlaybackUstreamerConfig = playerResponse.player_config?.media_common_config?.media_ustreamer_request_config
    ...
    const clientInfo = playerClient === 'YTMUSIC'
        ? {
    ...
    sabrStream.on('reloadPlayerResponse', async (reloadPlaybackContext) => {
        try {
            const { playerResponse: newInfo } = await fetchStreamingPlayerResponse(innertube, videoId, playerClient, reloadPlaybackContext);
